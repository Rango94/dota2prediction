...
for line in initdata:
    ...
    if filtration(line):
        ...
        newout.append(out)
        if len(out)==10:
            newout.append(findsubstrnum('"gold_per_min":',line))

        if findsubstrnum1('"radiant_win":', line)==-1:
            for i in range(0,10):
                if i<5:
                    gold[0][int(newout[0][i])-1]=gold[0][int(newout[0][i])-1]+newout[1][i]
                    gold[1][int(newout[0][i]) - 1] = gold[1][int(newout[0][i]) - 1] + 1
                else:
                    gold[2][int(newout[0][i]) - 1] = gold[2][int(newout[0][i]) - 1] + newout[1][i]
                    gold[3][int(newout[0][i]) - 1] = gold[3][int(newout[0][i]) - 1] + 1
        if findsubstrnum1('"radiant_win":', line) == 1:
            for i in range(0, 10):
                if i < 5:
                    gold[2][int(newout[0][i]) - 1] = gold[2][int(newout[0][i]) - 1] + newout[1][i]
