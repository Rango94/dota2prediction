...
def guolv(ss):
    ...
    if outt[0]>=3000 and outt[0]<30000  :
        gr=0
        gd=0
        if outg!=0:
            for i in range(0,10):
            ...
            if outr != False and outd != False and outr[0] != 0 and outd[0] != 0:
                if int(outr[0]) > int(outd[0]):
                    if float((int(outr[0]) - int(outd[0])) / int(outd[0])) < 0.4 :
                        #or gr - gd < 0.4 * gd
                        return False
                else:
                    if float((int(outd[0]) - int(outr[0])) / int(outr[0])) < 0.4 :
                        #or gd - gr < 0.4 * gr
                        return False
            else:
                return False
...
for line in fo3:
    ...
    dy.append(line)
file=[]
file.append(open('highnewdata1.txt'))
file.append(open('highnewdata.txt'))
file.append(open('highnewdata2.txt'))
fo1=open('cleardatabig8888.txt','w')
